...
class CategoryController extends PublicController {
    ...
    public function index(){
    	$list = M('category')->where('tid=1 AND is_show=1')->field('id,tid,name,enname')->order("sort desc,id asc")->select();
        $catList = M('category')->where('tid='.intval($list[0]['id']))->field('id,name,bz_1,enname')->select();
    ...
    public function getcat(){
        ...
        $catList = M('category')->where('tid='.intval($catid))->field('id,name,bz_1,enname')->select();
        foreach ($catList as $k => $v) {
            $catList[$k]['bz_1'] = __DATAURL__.$v['bz_1'];
        }
