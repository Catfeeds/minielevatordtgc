...
class WebController extends PublicController {
    ...
    public function web(){
    	...
    	$content = M('web')->where('id='.intval($web_id))->getField('content');
    ...
    public function aboutUs(){
        $web=M("web");
        $list=array();
        $weblist=$web->field("id,uname,ename")->order("sort desc")->select();
        foreach ($weblist as $k => $v) {
            $nodeslist[$k]="content_".$v['id'].".nodes";
        }
        echo json_encode(array('status'=>1,'err'=>$weblist,"nodeslist"=>$nodeslist));
    }

    public function getcontent(){
        $id=I("request.id");
        $re=M("web")->where("id=$id")->getField("content");
        $content = str_replace(C('content.dir'), __DATAURL__, $re);
        $content= html_entity_decode($content, ENT_QUOTES ,'utf-8');
        echo json_encode(array("status"=>1,"err"=>$content));
    }
    //***************************
    //  产品商家搜索接口
    //***************************
